...
export const MINI_IPIP_QUESTIONS: string[] = [
...
export const REVERSED_ITEMS = new Set<number>([2, 3, 4, 6, 10, 12, 13, 14, 16, 20]);
...
export interface BigFiveScores {
  ...
  N: number;
  O: number;
  S: number; // 1..5 (stabilność emocjonalna)
  norm: { E: number; A: number; C: number; O: number; S: number }; // 0..1
}
...
export interface SurveyResult {
  answers: Likert[]; // długość 20
  bigFive: BigFiveScores;
  roles: RoleScore[]; // posortowane malejąco
  topRoles: RoleScore[]; // 3 najlepsze
  completedAt: string; // ISO
}

function reverseLikert(x: Likert): Likert {
  // 6 - odpowiedź
  return (6 - x) as Likert;
}
...
function norm01(x15: number): number {
  // (x - 1)/4
  return (x15 - 1) / 4;
}

export function computeBigFive(rawAnswers: Likert[]): BigFiveScores {
  if (rawAnswers.length !== 20) throw new Error("Oczekiwano 20 odpowiedzi.");

  // odpowiedzi po odwróceniach
  const adj: number[] = rawAnswers.map((v, idx) => {
    const itemNo = idx + 1;
...
export function computeRoleScores(norm: BigFiveScores["norm"]): RoleScore[] {
...
export function computeSurveyResult(answers: Likert[]): SurveyResult {
